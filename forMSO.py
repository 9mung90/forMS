...
# --- 수정된 부분: CPU만 사용하도록 강제 ---
device = torch.device("cpu") # 학습할 때 CPU만 쓰라고 설정하는 부분
...
def preprocess_data(examples): # 데이터를 모델이 쓰기 좋은 형태로 바꿔주는 함수로 정의함
    ...
    # 다중 라벨 분류를 위해 각 라벨을 float 리스트로 변환 [1, 0, 0, 1, 0, 0, 0, 0] 이런 식으로
    labels = [
...
model.to(device) # 만들어진 모델을 위에서 설정한 CPU로 보냄
...
for epoch in range(num_epochs): # 정해진 에포크만큼 반복
    for batch in train_dataloader: # 데이터로더에서 배치(16개 묶음)를 하나씩 꺼내옴
        ...
        batch = {k: v.to(device) for k, v in batch.items()} # 배치를 통째로 CPU로 보냄
...
def classify_sentence(sentence): # 새로운 문장이 들어오면 혐오 표현인지 아닌지 판단하는 함수
    ...
    inputs = {k: v.to(device) for k, v in inputs.items()} # 토큰화된 데이터도 CPU로 보냄
